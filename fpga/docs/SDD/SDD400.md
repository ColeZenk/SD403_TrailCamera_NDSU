...
```
top.v
|---- esp_interface.v       (SPI slave RX + MISO TX path)
...
|---- gowin_rpll.v          (PLL, not used -- sys_clk = 27 MHz direct)
|---- block_sched           (blk_schd.v -- monolithic WHT pipeline FSM)
|    |---- wht2d.v          (wht8 wrapper, combinational 8-point 1D)
|    |    \---- wht8.v      (3-stage butterfly, zero latency)
|    |---- seq_msk.v        (combinational sequency mask, r+c > T)
|    \---- coeff.v          (Top-K coefficient extractor FSM)
```

Note: The planned modular decomposition in SDD401 (blk_read.v, wht_2d.v, coeff_out.v,
somi.v, tx_fifo.v, psram_ctrl.v) was not implemented as described. See SDD401 for details.

## Data Flow Pipeline

```
ESP32-DevKit (SPI Master)
  | SPI (MOSI, SCLK, CS_n, MISO)
  v
esp_interface.v                     -- RX: byte stream in, MISO: coefficients out
  |---> BSRAM write              ---> lcd_controller.v -> RGB LCD (480x272)
  \---> PSRAM write (cur_base)

                    [CS rising edge -> cs_end -> block_sched.start]

block_sched.v (blk_schd.v)
  -- LOAD: reads cur_base + prev_base word pairs from PSRAM
  -- Diffs each pair, fills 64-entry pixel register array
  -- WHT: 16 passes (8 row + 8 col) via wht2d.v/wht8.v (combinational)
  -- seq_msk.v: zeros coefficients where r+c > 6 (combinational)
  -- WAIT: coeff.v FSM scans 64 positions, Top-5 by magnitude
  -- EMIT: 2 header bytes + 3 bytes per coefficient -> tx_buf in top.v
  -- NEXT: advance block counters, repeat for all 1200 blocks
  -- frame_done -> top.v swaps cur_base/prev_base

top.v tx_buf (512-byte BRAM)
  -- sched_tx_valid writes coefficients in as they arrive from block_sched
  -- esp_tx_next advances tx_rd_ptr; tx_rd_data feeds esp_interface.v tx_data
  -- ESP32 reads back coefficients via SPI (MISO path in esp_interface.v)
```

See [SDD401](./SDD401.md) for full WHT pipeline design and implementation notes.
...
**Purpose:** Full-duplex SPI slave -- RX path ingests image bytes, MISO TX path returns compressed coefficients.

**Implementation Details:**
- **Clock domain crossing:** 3-stage synchronizer on SCLK and CS_n; 2-stage on MOSI
- **Edge detection:** Rising (sample) and falling (shift) detection for SCLK
- **RX shift register:** 8-bit serial-to-parallel, byte_ready strobe
- **TX shift register (tx_sr):** Preloaded from tx_data on byte boundary and CS assert
- **MISO output:** tx_sr[7] gated by cs_active
...
**Known Bug (Bug 1):** Inter-byte MISO shift. After tx_sr is reloaded on byte_done_d,
the immediately following falling SCLK edge shifts tx_sr left by 1 before the master
samples the new MSB. Every byte[N>=1] received by ESP32 is the correct byte shifted
left by 1. Confirmed by esp_interface_tx_tb (22 failures). See UTR501.

**Current Status:** [DONE] Implemented -- Bug 1 open
...
## Actual Utilization (Synthesized)
| Module             | LUTs       | BRAM   | Status |
|--------------------|------------|--------|--------|
| LCD RGB Controller | ~800       | 0      | [X]    |
| SPI Slave (RX+TX)  | ~250       | 0      | [X]    |
| I2C GPIO Expander  | ~400       | 0      | [X]    |
| Control logic      | ~300       | 0      | [X]    |
| BRAM buffer (LCD)  | (inferred) | 16     | [X]    |
| tx_buf (top.v)     | (inferred) | 2      | [X]    |
| block_sched        | ~2000      | 0      | [X]    |
| wht8 (via wht2d)   | ~200       | 0      | [X]    |
| seq_msk            | ~50        | 0      | [X]    |
| coeff              | ~300       | 0      | [X]    |
| **Estimated Total**| **~4300**  | **18** | [X]    |

**Margin:** ~4340 LUTs remaining (50% used), 8 BRAM blocks unused. PSRAM is embedded (no LUT/BRAM cost).
Synthesis report in build/ is authoritative.
...
## Completed [DONE]
- [x] SPI slave RX path with 3-stage synchronizer, byte_ready strobe
- [x] SPI MISO TX path (tx_sr, preload from tx_data, byte_done_d reload)
- [x] BRAM image buffer for LCD (dual-port, 32KB)
- [x] PSRAM dual-write during SPI ingest (cur_base/prev_base ping-pong)
...
- [x] wht8.v -- 8-point combinational WHT butterfly (wht8_tb PASS)
- [x] wht2d.v -- wht8 bus wrapper for blk_schd
- [x] seq_msk.v -- combinational sequency mask (r+c > 6 -> zero)
- [x] coeff.v -- Top-K extractor FSM (thresh=400, q_shift=3, TOP_K=5, coeff_tb PASS)
- [x] blk_schd.v -- monolithic block scheduler FSM (IDLE/LOAD/WHT/WAIT/EMIT/NEXT)
- [x] top.v tx_buf (512-byte BRAM, registered read, tx_rd_ptr, tx_len)
- [x] Frame diff (FPGA-side, cur_base vs prev_base)

## Open Bugs
- [ ] Bug 1: MISO inter-byte shift in esp_interface.v -- 22 sim failures (UTR501 TB003)
- [ ] Bug 2: blk_schd_tb PSRAM mock deadlock -- testbench hangs (UTR501 TB004)

## Future Work [TODO]
- [ ] Fix Bug 1: MISO TX pipeline skew
- [ ] Fix Bug 2: rewrite blk_schd_tb PSRAM mock with explicit cycle counter
...
## Integration Testing
- **End-to-end:** ESP32 -> SPI -> FPGA -> LCD (functional, camera stream visible)
- **WHT Pipeline:** wht8_tb PASS, coeff_tb PASS; blk_schd_tb blocked by testbench bug (UTR501)
...
1. **Bug 1 -- MISO inter-byte shift:** esp_interface.v tx_sr is reloaded on byte_done_d,
   but the next falling SCLK edge (the between-bytes transition) immediately shifts it left
   by 1. Every byte[N>=1] received by the ESP32 equals the correct byte shifted left by 1
   (MSB appears as 0, bit 0 of the correct byte is lost). Confirmed by sim, 22 failures.
   Fix: gate the shift on bit_count != 0 (skip the first falling edge after reload).
   See UTR501 TB003.

2. **Bug 2 -- blk_schd_tb PSRAM mock deadlock:** The testbench PSRAM model clears
   psram_busy in the same cycle that blk_schd can fire a second read (when
   psram_busy reads as stale 0). blk_schd clears psram_pending before the mock
   delivers data, leaving psram_busy stuck at 1 permanently. Simulation hangs.
   Fix: rewrite mock with an explicit N-cycle countdown rather than edge detection.
   See UTR501 TB004.

3. **top.v tx_buf registered-read lag:** BRAM read has 1-cycle latency. When
   byte_done_d fires to reload tx_sr, tx_rd_data still holds the byte from the
   previous tx_rd_ptr value. tx_rd_ptr must advance one cycle before the reload
   fires, or tx_sr preload must be delayed by one additional cycle.

4. **ESP32 heap corruption (cam_spi buffer leak):** 76800-byte DMA buffers are
   leaked on RX checksum failure. Over time this corrupts the heap region holding
   ctx.mutex, causing an xSemaphoreGive assert crash at fpga_spi.c:192.
   Fix: call safe_free in cam_spi rx_reset path before realloc.
