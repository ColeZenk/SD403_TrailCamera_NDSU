...
module esp_interface
    (input wire clk,
     ...
     output wire cs_active_sync // synchronized CS active level for top.v
        ...
        // AND-mask: shifting selects new shifted value, ~shifting holds
        // current ~cs_active zeros everything (overrides shifting since
        // shifting requires cs_active)
        wire [7 : 0] shift_next = (shifted_in & {8{shifting}}) |
                                  (shift_reg & {8{~shifting & cs_active}});
        wire [2 : 0] count_inc = (bit_count + 3'd1) &
                                 {3{~byte_done}}; // wraps to 0 on byte_done
        wire [2 : 0] count_next = (count_inc & {3{shifting}}) |
                                  (bit_count & {3{~shifting}});
        wire [2 : 0] count_out = count_next &
                                 {3{cs_active}}; // zero when CS deasserted
        ...
        // ==========================================================
        wire [7 : 0] captured = {shift_reg[6 : 0],
                                 mosi_sync[1]}; // include last bit
        wire [7 : 0] data_next = (captured & {8{byte_ready}}) |
                                 (rx_data & {8{~byte_ready}});
