 ...
 *   0x02  DIR_REG       1=input, 0=output  (default 0xFF — all inputs on
 * reset) 0x03  INVERT_REG    Input polarity invert mask (default 0x00)
 *
...
module i2c_gpio_expander
    ...
    (input wire clk,
     ...
     input wire sda_i,
     output reg sda_oe,
     // 1 = drive SDA low; 0 = release (pull-up handles high)

     output reg [7 : 0] gpio_out,
     // output register — wire to physical pins via dir gate
     output reg [7 : 0] gpio_dir,
     // direction: 1=input, 0=output
     output reg [7 : 0] gpio_invert,
     // input invert mask
     input wire [7 : 0]
         gpio_in // physical input pin values (registered externally)
    );
        ...
        wire start_det = scl_sync[1] &
                         (sda_sync[2 : 1] == 2'b10); // SDA falls, SCL high
        wire stop_det = scl_sync[1] &
                        (sda_sync[2 : 1] == 2'b01); // SDA rises, SCL high
        ...
        reg [7 : 0]
            reg_addr; // current register pointer (auto-increments on burst)
                ...
                begin
                        case (addr)
                                ...
                                8'h01:
                                        read_reg =
                                            gpio_in ^
                                            gpio_invert; // invert applied, dir
                                                         // not masked (master
                                                         // knows dir)
                                8'h02:
                                        read_reg = gpio_dir;
                                8'h03:
                                        read_reg = gpio_invert;
                                default:
                                        read_reg = 8'hFF;
                        endcase
                end
        ...
        //
        // Timing contract:
        //   ACK/data bits driven on scl_fall (low phase), stable when SCL
        //   rises RX bits sampled on scl_rise START/STOP always override
        //   current state
        // ================================================================
        always @(posedge clk or negedge rst_n) begin
                if (!rst_n) begin
                        ...
                        gpio_out <= 8'h00;
                        gpio_dir <=
                            8'hFF; // all inputs on reset — safe default
                        gpio_invert <= 8'h00;
                        shift_reg <= 8'h00;
                        bit_cnt <= 3'd0;
                        rw_bit <= 1'b0;
                        ...
                        if (start_det) begin
                                ...
                                case (state)
                                        ...
                                        // ------------------------------------------------
                                        // Shift in address byte (7 addr bits +
                                        // R/W)
                                        S_ADDR: begin
                                                if (scl_rise) begin
                                                        shift_reg <=
                                        ...
                                        // ------------------------------------------------
                                        // Drive ACK low during SCL low phase
                                        // after address. Hold sda_oe through
                                        // SCL high — releasing here would let
                                        // SDA rise while SCL is still high,
                                        // which stop_det would misread as a
                                        // STOP condition.  The destination
                                        // state (S_REG / S_READ) releases
                                        // sda_oe on its first scl_fall (SCL
                                        // low), so no false STOP is generated.
                                        S_ADDR_ACK: begin
                                                if (scl_fall) begin
                                                        if (shift_reg[7 : 1] ==
                                                            DEVICE_ADDR) begin
                                        ...
                                        // ------------------------------------------------
                                        // Latch write on scl_fall after 8th
                                        // bit
                                        S_WRITE_ACK: begin
                                                if (scl_fall) begin
                                                        // commit the byte to
                                        ...
                                        // Drive on scl_fall, stable during SCL
                                        // high
                                        S_READ: begin
                                                if (scl_fall) begin
                                                        sda_oe <=
                                        ...
                                        // ACK (SDA low)  = master wants more
                                        // bytes NACK (SDA high) = master done
                                        S_READ_ACK: begin
                                                if (scl_fall)
                                                        sda_oe <=
                                                            1'b0; // release
