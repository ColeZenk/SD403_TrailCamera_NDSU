...
static esp_err_t init_system(void)
{
        ...
        static const i2c_bus_config_t i2c_cfg = {
            ...
            .enable_internal_pullup =
                false, /* external 4.7kΩ pull-ups fitted */
        };
        ...
        if (ret != ESP_OK)
                ESP_LOGW(TAG,
                         "FPGA GPIO expander unavailable (%s) — I2C buttons "
                         "disabled",
                         esp_err_to_name(ret));
...
static void create_tasks(void)
{
        ...
        xTaskCreate(fpga_test_task, "fpga_test", STACK_SIZE_MEDIUM, NULL,
                    TASK_PRIORITY_MEDIUM, NULL);
#endif
#ifdef TEST_MODE_FPGA_GPIO
        xTaskCreate(fpga_gpio_test_task, "fpga_gpio_test", STACK_SIZE_MEDIUM,
