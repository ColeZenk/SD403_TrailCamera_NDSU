...
esp_err_t fpga_spi_transmit(const uint8_t *data, size_t size)
{
        ...
        while (offset < size || in_flight > 0) {
                ...
                while (offset < size && in_flight < FPGA_SPI_QUEUE_SIZE) {
                        ...
                        size_t chunk =
                            min_size(size - offset, FPGA_MAX_TRANSFER_SIZE);

                        spi_transaction_t *t =
                            &trans_pool[pool_idx % FPGA_SPI_QUEUE_SIZE];
                        t->length = chunk * 8;
                        t->rxlength =
                            0; /* reset: driver may set this on previous use */
                        t->tx_buffer = data + offset;
                        t->rx_buffer = NULL;
                        t->flags = 0;

                        ret = spi_device_queue_trans(ctx.device, t,
                                                     portMAX_DELAY);
...
esp_err_t fpga_spi_init(void)
{
        ...
        esp_err_t ret =
            spi_bus_initialize(FPGA_SPI_HOST, &bus, FPGA_SPI_DMA_CHAN);
        if (ret != ESP_OK) return ret;
