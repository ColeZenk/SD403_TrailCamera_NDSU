...
typedef enum { RX_IDLE, RX_DATA, RX_COMPLETE } rx_state_t;
...
static bool rx_process_header(const uint8_t *data, size_t len)
{
        ...
        if (!ctx.buffer) {
                ESP_LOGE(TAG, "alloc failed: %" PRIu32 " bytes",
                         ctx.header.size);
                return false;
        }
...
static esp_err_t spi_receive_chunk(size_t *out_len, uint32_t timeout_ticks)
{
        ...
        if (ret == ESP_OK) { *out_len = trans.trans_len / 8; }
...
esp_err_t cam_spi_init(void)
{
        ...
        esp_err_t ret =
            spi_slave_initialize(CAM_SPI_HOST, &bus, &slave, CAM_SPI_DMA_CHAN);
        if (ret != ESP_OK) {
        ...
        ESP_LOGI(TAG, "ready — MOSI=%d MISO=%d CLK=%d CS=%d", CAM_PIN_MOSI,
                 CAM_PIN_MISO, CAM_PIN_SCLK, CAM_PIN_CS);
        return ESP_OK;
}
...
QueueHandle_t cam_spi_get_queue(void) { return ctx.queue; }
...
void cam_spi_receive_task(void *pvParameters)
{
        ...
        for (;;) {
                size_t chunk_len = 0;
                esp_err_t ret =
                    spi_receive_chunk(&chunk_len, CAM_TRANSFER_TIMEOUT_TICKS);
