...
esp_err_t fpga_gpio_init(void)
{
        ...
        if (err != ESP_OK) {
                ESP_LOGE(TAG, "DIR_REG write failed: %s",
                         esp_err_to_name(err));
                return err;
        }

        /* Polarity invert: bits[2:0] inverted so 1=pressed for active-low
         * buttons */
        err = write_reg(FPGA_GPIO_REG_INV, FPGA_GPIO_INV_BOOT);
        if (err != ESP_OK) {
                ESP_LOGE(TAG, "INV_REG write failed: %s",
                         esp_err_to_name(err));
                return err;
        }
        ...
        if (err != ESP_OK) {
                ESP_LOGE(TAG, "OUT_REG clear failed: %s",
                         esp_err_to_name(err));
                return err;
        }
...
void fpga_gpio_test_task(void *pvParameters)
{
        ...
        for (;;) {
                ...
                if (buttons != prev_buttons) {
                        ...
                        /* L = prev pattern, R = next pattern, S = reset to
                         * gradient */
                        if (btn_l)
                                pat = (bpat_t)((pat + BPAT_COUNT - 1) %
                                               BPAT_COUNT);
                        ...
                        if (btn_l || btn_r || btn_s) {
                                ...
                                esp_err_t tx =
                                    fpga_spi_transmit(buf, TEST_IMAGE_SIZE);
                                if (tx == ESP_OK)
                                        ESP_LOGI(TAG, "sent pattern: %s",
                                                 bpat_names[pat]);
                                else
                                        ESP_LOGE(TAG, "SPI tx failed: %s",
                                                 esp_err_to_name(tx));
                        }
