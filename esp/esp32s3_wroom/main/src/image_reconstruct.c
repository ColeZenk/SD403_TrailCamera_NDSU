...
uint8_t *reconstruct(const uint8_t *packet, size_t pkt_len,
        ...
        *position_out = pos_idx;
        ...
        for (int i = 0; i < bbox_blocks; i++)
                if ((bitmap[i >> 3] >> (7 - (i & 7))) & 1) active_count++;
        ...
        for (int row = 0; row < bbox_h; row++) {
                for (int col = 0; col < bbox_w; col++) {
                        ...
                        if (!((bitmap[bit >> 3] >> (7 - (bit & 7))) & 1))
                                continue;

                        const uint8_t byte = dc_data[nibble_idx >> 1];
                        const uint8_t nibble =
                            (nibble_idx & 1) ? (byte & 0x0F) : (byte >> 4);
        ...
        for (int py = py1; py < py2 && py < FRAME_H; py++) {
                for (int px = px1; px < px2 && px < FRAME_W; px++) {
                        ...
                        for (int k = 0; k < block_idx; k++) {
                                const float cx = blocks[k].bx * BLOCK_SIZE +
                                                 BLOCK_SIZE / 2.0f;
                                const float cy = blocks[k].by * BLOCK_SIZE +
                                                 BLOCK_SIZE / 2.0f;
                                const float dx = px - cx;
                                const float dy = py - cy;

                                /* Skip blocks outside Gaussian radius */
                                if (fabsf(dx) > radius_px ||
                                    fabsf(dy) > radius_px)
                                        continue;

                                const float w =
                                    expf(-(dx * dx + dy * dy) * sigma2_inv);
                                wsum += w * blocks[k].dc;
                                wtotal += w;
                        }
                        ...
                        if (wtotal > 0.0f) {
                                ...
                                const int val =
                                    (int)ref[idx] + (int)(wsum / wtotal);
                                ref[idx] = (uint8_t)(val < 0     ? 0
                                                     : val > 255 ? 255
                                                                 : val);
                        }
                }
