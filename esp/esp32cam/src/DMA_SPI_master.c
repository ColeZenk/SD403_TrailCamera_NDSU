...
typedef struct {
        ...
        SemaphoreHandle_t
            bus_mutex; /* held for full duration of each transfer */
        bool initialized;
} spi_dma_context_t;
...
esp_err_t spi_dma_send_image(const uint8_t *image_data, size_t image_size) {
        ...
        /* Re-check under mutex: deinit may have run between the check above and
         * lock */
        if (!ctx.initialized) {
                xSemaphoreGive(ctx.bus_mutex);
...
void spi_transmit_task(void *pvParameters) {
        ...
        for (;;) {
                if (xQueueReceive(image_queue, &img_data, portMAX_DELAY) ==
                    pdTRUE) {
                        esp_err_t ret =
