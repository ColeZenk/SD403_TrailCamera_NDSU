...
void app_main(void) {
        ...
        /* SPI DMA master — pins 13/14 (shared with SD, time-shared in capture
         * loop)
         */
        ...
        QueueHandle_t image_queue =
            xQueueCreate(IMAGE_QUEUE_DEPTH, sizeof(image_data_t));
        if (image_queue == NULL) {
        ...
        xTaskCreate(spi_transmit_task, "spi_tx", 4096, (void *)image_queue, 5,
                    NULL);
