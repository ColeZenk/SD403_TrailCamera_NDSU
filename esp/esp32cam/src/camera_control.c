 ...
 *   - SD save: JPEG every SD_SAVE_INTERVAL_FRAMES frames (pin time-shared with
 * SPI)
 *
...
static void save_jpeg_to_sd(const uint8_t *raw, size_t raw_len, uint16_t w,
                            uint16_t h) {
        uint8_t *jpg = NULL;
        size_t jpg_len = 0;

        if (!fmt2jpg((uint8_t *)raw, raw_len, w, h, PIXFORMAT_GRAYSCALE, 80,
                     &jpg, &jpg_len)) {
                ESP_LOGE(TAG, "JPEG encode failed");
                return;
        }

        ESP_LOGI(TAG, "JPEG: %zu bytes (%.1f:1)", jpg_len,
                 (float)raw_len / jpg_len);
...
static void capture_loop(void *arg) {
        ESP_LOGI(TAG,
                 "Capture loop started — target %d fps, SD every %d frames",
        ...
        for (;;) {
                ...
                if (frame_count % SD_SAVE_INTERVAL_FRAMES == 0)
                        save_jpeg_to_sd(fb->buf, fb->len, fb->width,
                                        fb->height);

                esp_camera_fb_return(fb);

                /* Pace to FRAME_PERIOD_MS — sleeps the remainder of the period.
                 * If capture + SPI overruns the budget, runs immediately next
                 * cycle. */
