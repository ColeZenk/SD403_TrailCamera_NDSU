 ...
 * The rest of the codebase uses ov2640_cfg_t structs.
 */
...
static sensor_t *get_sensor(void)
{
        ...
        if (!s) {
                ESP_LOGE(
                    TAG,
                    "sensor handle is NULL — was esp_camera_init() called?");
        }
        return s;
}
 ...
 */
static esp_err_t tune_exposure(sensor_t *s, int target, int tolerance,
                               int initial_aec)
{
        int aec = initial_aec;
        ...
        for (int i = 0; i < 20; i++) {
                ...
                ESP_LOGI(TAG, "tune [%2d]: aec=%-4d  mean=%-3d  target=%d", i,
                         aec, mean, target);
...
esp_err_t ov2640_apply_config(const ov2640_cfg_t *cfg)
{
        ...
        if (cfg->target_mean > 0) {
                tune_exposure(s, cfg->target_mean, cfg->tune_tolerance,
                              cfg->exposure);
        } else {
